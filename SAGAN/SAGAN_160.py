...
batch_size = 8  # Reduced for 160x160 (higher memory usage)
learning_rate = 0.0001  # Lower learning rate for higher resolution
num_epochs = 3000  # More epochs for higher resolution convergence
image_size = 160  # Image size (160x160) - HIGH RESOLUTION
latent_dim = 128  # Increased latent dimension for more diversity
nc = 1  # Number of channels (1 for grayscale, 3 for RGB)
ngf = 64  # Number of generator filters (keep at 64 for balance)
ndf = 64  # Number of discriminator filters (keep at 64 for balance)
...
target_dir = './real_images_160x160_for_fid'   # 160x160 评估集（直接使用原始分辨率）
real_data_path = './real_images_160x160_for_fid'
...
# 定义变换：保持 160x160，只做灰度转换
resize_transform = transforms.Compose([
    # 步骤 1: 确保是 160x160（如果原始就是160x160则不变）
    transforms.Resize((160, 160), interpolation=transforms.InterpolationMode.BICUBIC),
    
    # 步骤 2: 转换为灰度图
    transforms.Grayscale(num_output_channels=1)
])
print(f"正在从 {source_dir} 创建 160x160 评估集于 {target_dir}...")
...
print(f"--- 完成！---")
print(f"总共 {count} 张 160x160 的图片被保存到了 {target_dir}")
...
class Generator(nn.Module):
    """
    SAGAN Generator (Self-Attention GAN) - 160x160 HIGH RESOLUTION
    Architecture: DCGAN backbone with Self-Attention at 40x40 resolution
    Input: latent vector z of dimension (latent_dim, 1, 1)
    Output: Generated image of size (nc, 160, 160)
    Reference: Zhang et al. "Self-Attention Generative Adversarial Networks" (2018)
    
    Resolution progression: 1x1 → 5x5 → 10x10 → 20x20 → 40x40 → 80x80 → 160x160
    """
    def __init__(self):
        ...
        # Add Self-Attention at 40x40 resolution (middle resolution for 160x160)
    ...
    def forward(self, input):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        x = self.attention(x)  # Apply self-attention at 40x40
        x = self.layer5(x)
        x = self.layer6(x)
        return x
...
class Discriminator(nn.Module):
    """
    SAGAN Discriminator (Self-Attention GAN) - 160x160 HIGH RESOLUTION
    Architecture: DCGAN backbone with Self-Attention at 40x40 + Spectral Normalization
    Input: Image of size (nc, 160, 160)
    Output: Single scalar value (raw logit for Hinge Loss)
    Reference: Zhang et al. "Self-Attention Generative Adversarial Networks" (2018)
    
    Resolution progression: 160x160 → 80x80 → 40x40 → 20x20 → 10x10 → 5x5 → 1x1
    """
    def __init__(self):
        ...
        self.layer6 = nn.Sequential(
            nn.utils.spectral_norm(nn.Conv2d(ndf * 16, 1, 5, 1, 0, bias=False))
...
summary(discriminator, (1,160,160))
...
with torch.no_grad():
    for i in range(num_images):
        ...
        # 生成器直接输出 160x160，不需要放大
        # 直接保存，使用 save_image 并指定 value_range
...
print(f"Generated {num_images} images (native 160x160) in {generated_images_dir}")
