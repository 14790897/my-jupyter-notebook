...
SEGMENT_DURATION = 10  # 每段视频时长(秒)，测试用
TEST_SEGMENTS = 0  # 只处理前N段，测试用（设为 None 处理全部）
...
SUBTITLE_FONT_SIZE = 32  # 减小字体防止长文本溢出
...
# 获取视频实际分辨率（PlayResX/Y 必须匹配视频，否则字幕位置会错位）
...
video_w = int(vs["width"])
video_h = int(vs["height"])
...
def wrap_ass_text(text, chars_per_line=100):
    """将长文本按固定字符数插入 ASS 换行符 \\N"""
    lines = text.split("\n")
    wrapped = []
    for line in lines:
...
with open(ass_path, "w", encoding="utf-8") as f:
    ...
    # Alignment=8: 顶部居中; PrimaryColour=&H00FFFF00=黄色; OutlineColour=黑色描边
    ...
    for r in results:
        ...
        # 先换行，再转义逗号
        text = wrap_ass_text(r["text"], chars_per_line=70)
        text = text.replace(",", "\\,")
