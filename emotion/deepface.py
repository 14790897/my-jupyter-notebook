...
clear_output()
# %% [code]
input_file = "/kaggle/input/datasets/liuweiq/daxiaonailong/Ngana Rindu.mp4"
...
RACE_COLORS = {
    'asian': (255, 128, 0),         # 蓝紫色
    'white': (255, 200, 124),       # 浅蓝
    'black': (0, 0, 139),           # 深红
    'middle eastern': (255, 105, 180),  # 粉红
    'indian': (200, 100, 50),       # 棕色
    'latino hispanic': (0, 200, 200)  # 黄色
}
...
def process_video_every_frame(input_path, output_path):
        ...
        try:
            ...
            if results:
                for face_data in results:
                    ...
                    age = face_data['age']
                    # 获取种族信息（取置信度最高的种族）
                    race_scores = face_data['race']
                    raw_race = max(race_scores, key=race_scores.get)
                    race_score = race_scores[raw_race]
    ...
    # --- 新增：获取当前情绪的置信度分数 ---
                    ...
                    if best_match:
                        ...
                        # 将种族及其置信度打包存入队列
                        best_match['races'].append((raw_race, race_score))
                        ...
                        # --- 种族置信度累加判断 ---
                        race_weights = {}
                        for rc, score in best_match['races']:
                            race_weights[rc] = race_weights.get(rc, 0) + score
                        # 取累积置信度最高的种族
                        final_race = max(race_weights, key=race_weights.get)
                        ...
                        new_track = {
                            ...
                            'races': deque([(raw_race, race_score)], maxlen=SMOOTH_FRAMES)
                        }
                    ...
                    # 提取并翻译最终的平滑种族
                    cn_race = RACE_DICT.get(final_race, '未知')
